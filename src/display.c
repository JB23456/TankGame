...
void printTextFlipped(const char *Text,uint16_t x, uint16_t y, uint16_t ForeColour, uint16_t BackColour)
{
    // This is an edited copy to print the text upside down, text parameter should be passed in reverse
	// This function draws each character individually.  It uses an array called TextBox as a temporary storage
    // location to hold the dots for the character in question.  It constructs the image of the character and then
    // calls on putImage to place it on the screen
    uint8_t Index = 0;
    ...
    for (Index = 0; Index < len; Index++)
    {
        ...
        putImage(x, y, FONT_WIDTH, FONT_HEIGHT, (uint16_t *)TextBox,1,1);
        x = x + FONT_WIDTH + 2;
    }
}
...
{
    // R, G and B are hex values, 0x00 -> 0xFF
	uint16_t rvalue = 0;
    rvalue += G >> 5;
    rvalue += (G & (7)) << 13;
    rvalue += (R >> 3) << 8;
